...
| S5a exact search and acknowledged receipt-journal seam | [Issue #34 S5a evidence](ISSUE_34_S5_EXACT_SEARCH_RECEIPT_JOURNAL.md) | `packages/server/src/artifact-inspector.test.ts`, `packages/server/src/artifact-mcp-registration.test.ts`, and `packages/server/src/artifact-receipt-journal.test.ts` | `526c42c01f1e861664fd155c4110a7a6a6d41302` |

## Unmerged candidate evidence

| Scope | Evidence | Executable seam | Notes |
| --- | --- | --- | --- |
| Experimental local remote HTTP + OAuth 2.1 | [Issue #62 evidence](ISSUE_62_REMOTE_OAUTH_HTTP.md) | `packages/server/src/remote-http-profile.test.ts`, `packages/contracts/src/remote-oauth-http-policy.test.ts`, `packages/server/src/local-oauth-pkce.e2e.test.ts`, and `npm run test:remote-oauth` | Fail-closed Data API dispatch. [ADR-0006](../decisions/0006-downstream-credential-recheck.md) rechecked 2026-09-22: still no supported downstream credential. Hosted live OAuth unmet. Not #62 completion. |
