...
- The fixed RPC is the intended authorization and RLS boundary. This application-only slice proves that it receives and maps only returned rows and never accepts or computes a pre-authorization candidate set or total; mocked transport does not prove the future RPC implementation or its RLS behavior.
...
- Contract filter, cursor, row, UTF-8 MCP wire-response byte, and 2-second execution ceilings are enforced. The deadline covers headers and streamed-body consumption. Both caller cancellation and the deadline abort the fixed request.
...
Focused RED was observed before implementation: `memory-search.ts` was unresolved and `searchMemoryRows` did not exist. After implementation, mocked tests cover positive mapping; strict unknown-key, malformed query, filter, limit, and cursor denial; identity/origin/schema/RPC override denial; mapping-only-before-ranking and no hidden counts; output row/byte boundaries; cancellation and stalled-body timeout; prompt injection as untrusted data; and fixed path/JWT/schema/method/limit assertions.
